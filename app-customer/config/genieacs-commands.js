...
async function handleRestartDevice(remoteJid, senderNumber) {
    ...
    try {
        ...
        await sock.sendMessage(remoteJid, {
            text: formatResponse('Terjadi kesalahan saat memulai ulang perangkat.')
        });
        }
        }

        // Handler for auto bridge command
        async function handleAutoBridge(remoteJid, senderNumber) {
        if (!sock) {
        logger.error('Sock instance not set');
        return;
        }
        ...
        // Send processing message
        await sock.sendMessage(remoteJid, {
            text: formatResponse('Sedang mengatur perangkat ke mode Bridge...')
        });

        // Set bridge mode
        await genieacsApi.autoBridge(device._id);

        // Send success message
        await sock.sendMessage(remoteJid, {
            text: formatResponse('Perangkat berhasil diatur ke mode Bridge. Silakan restart perangkat jika perlu.')
        });

        } catch (error) {
        logger.error(`Error handling auto bridge: ${error.message}`);
        await sock.sendMessage(remoteJid, {
            text: formatResponse('Terjadi kesalahan saat mengatur mode Bridge.')
        });
        }
        }
