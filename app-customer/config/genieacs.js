...
const genieacsApi = {
    ...
    async autoBridge(deviceId) {
        try {
            console.log('Setting auto bridge for device:', deviceId);

            // Set both PPP and IP connection to bridge mode
            const parameterValues = [
                ["InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.WANPPPConnection.1.ConnectionType", "PPPoE_Bridged"],
                ["InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.WANIPConnection.1.ConnectionType", "IP_Bridged"]
            ];
            ...
            // Refresh WAN connection to update status
            await axiosInstance.post(
                `/devices/${encodeURIComponent(deviceId)}/tasks`,
                {
                    name: "refreshObject",
                    objectName: "InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1"
                }
            );

            return response.data;
        } catch (error) {
            console.error(`Error setting auto bridge for device ${deviceId}:`, error.response?.data || error.message);
            throw error;
        }
    },
...
module.exports = {
    ...
    autoBridge: genieacsApi.autoBridge,
