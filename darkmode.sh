...
# --- NOC INTELLIGENCE UI INJECTION (v16.0 RESPONSIVE-ELITE) ---
type_out "${CYAN}>>> Melakukan Ultra-Stable Transformation UI v16.0..."
...
if [ -f "$REPO_APP_JS" ]; then
    for f in "$SYSTEM_PUBLIC"/app*.js; do
        if [ -f "$f" ]; then
            echo "Applying v16.0 Responsive-Elite: $f"
            sudo sed -i '/\/\*\* NOC Intelligence/d' "$f"
            sudo tee -a "$f" < "$REPO_APP_JS" > /dev/null
        fi
    done
    sudo systemctl restart genieacs-ui
    echo -e "${GREEN}✔ UI v16.0 Final Responsive-Elite Applied & Service Restarted.${NC}"
else
