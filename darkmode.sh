...
# --- NOC INTELLIGENCE UI INJECTION (v10.0 V-DOM GHOST) ---
type_out "${CYAN}>>> Melakukan Ultra-Stable Transformation UI v10.0..."
...
if [ -f "$REPO_APP_JS" ]; then
    for f in "$SYSTEM_PUBLIC"/app*.js; do
        if [ -f "$f" ]; then
            echo "Applying v10.0 V-DOM Ghost: $f"
            sudo sed -i '/\/\*\* NOC Intelligence/d' "$f"
            sudo tee -a "$f" < "$REPO_APP_JS" > /dev/null
        fi
    done
    sudo systemctl restart genieacs-ui
    echo -e "${GREEN}✔ UI v10.0 Final Enterprise Applied & Service Restarted.${NC}"
else
